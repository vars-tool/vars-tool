...
def run_optimization(folder,dates,metric,par_bounds,par_values,pn,pv, max_iter):
    ...
    print(pv)
    print(pn)
    output=minimize(error_fun,pv,args=(pn,par_values,metric,Qobs,watershed_area, ini_values, forcing, long_term,dates),method='Nelder-Mead', bounds=pb, options={'maxiter':max_iter})
    pv=output['x']
...
def run_optimization_mo(folder,dates,metric,par_bounds,par_values,pn,pv, max_iter, weights):
    ...
    coef1 = weights[0]
    coef2 = weights[1]
    output=minimize(error_fun_mo,pv,args=(pn,par_values,metric,Qobs,watershed_area, ini_values, forcing, long_term,dates, coef1, coef2),bounds=pb, options={'maxiter':max_iter}, method='Nelder-Mead')
    pv=output['x']
    for n,v in zip(pn,pv): par_values[n]=v
    print(par_values)
    return par_values, pv

def error_fun_mo(pv,pn,par_values,metric,Qobs,watershed_area, ini_values, forcing, long_term,dates, coef1, coef2):
    ...
    Q['Q']=Q_cms
    Q=Q[dates['start_calib']:dates['end_calib']].values
    err1=eval_metric(Qobs.values,Q,metric[0])
    err2=eval_metric(Qobs.values,Q,metric[1])
    err=(err1*coef1) + (err2*coef2)

    # Diplay current parameter values and objective function value:
    clear_output(wait = True)
    for n,v in zip(pn,pv): print('%s: %f'%(n,v))
    print('%s*%.2f + %s*%.2f: %.4f\n'%(metric[0],coef1,metric[1],coef2,err))
