...
def section_df(df:pd.DataFrame) -> pd.DataFrame:
	"""
	Description:
	------------
...
def variogram(pair_cols:pd.DataFrame) -> pd.DataFrame:
	...
	variogram_values = 0.5*(pair_cols[0] - pair_cols[1]).pow(2).groupby(level=['ts', 'param', 'h']).mean()
...
def e_covariogram(cov_section_all:pd.DataFrame) -> pd.DataFrame: 
	...
	e_covariogram_values = cov_section_all.groupby(level=['ts', 'param', 'h']).mean()
...
def sobol_eq(gamma:pd.DataFrame, ecov:pd.DataFrame, variance:pd.Series) -> pd.DataFrame:
	...
	sobol_eq_values = (gamma + ecov).div(variance, level='ts').loc[:, :, 1]
